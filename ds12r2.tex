...
\begin{figure}[h]
\includegraphics[width=8cm]{lag.eps}
\end{figure}
\end{slide}
\begin{slide}{ラグランジュ乗数による解}
拘束条件にラグランジュ乗数$\lambda$を掛け極値問題を拡大(augment). 
\begin{equation}
...
\begin{itemize}
\item $f(x,y) = x^2 - y^2 + 3$から$f_{xx} = 2, f_{yy} = -2, f_{xy} =0$、$\Delta = f_{xx}f_{yy} - f_{xy}^2<0$なので、制約条件がなければ極値は取らない
...
\begin{figure}[h]
\includegraphics[width=3cm]{paralag.eps}
\end{figure}
...
\begin{figure}[h]
\includegraphics[width=6cm]{lag2.eps}
\end{figure}
...
\end{itemize}
\begin{figure}[h]
\includegraphics[width=6cm]{lag3.eps}
\end{figure}
$f(x,y)$は平面
\end{slide}
...
\end{itemize}
\begin{figure}[h]
\includegraphics[width=10cm]{paraint.eps}
\end{figure}
\end{slide}
...
と書き換えられるので、以下の半円である。
\begin{figure}[h]
\includegraphics[width=2.5cm]{iint.eps}
\end{figure}
\begin{eqnarray}
...
\begin{slide}{Quiz}
\begin{itemize}
\item 開始するとSOLにQuizが表示されます。
\item 提出（submit)は一回しかできないので、よく見直してから提出すること。
\item 時間は15分です。提出しても時間までは静かに着席していてください。
\item 授業資料を見ることや、ネットで調べるのはOKです。自分でプログラムを書いて答えを求めてもいいです。
\item 隣の友達と相談したり、ネットで質問して答えを教えてもらってはいけません。
\item 途中で「ミュート」になってしまったら、Quiz終了後に教えてください。
\end{itemize}
\end{slide}
\begin{slide}{Quiz}
指示に従い計算せよ。
\begin{itemize}
\item 全角文字は使わない。
\item 数値は既約分数を用いて書くこと。
\end{itemize}
\end{slide}
\begin{slide}{Quiz12-1}
今日のキーワードを記入せよ
\end{slide}
\begin{slide}{Quiz12-2}
\begin{itemize}
\item $f(x,y) = x^2 + y^2$に$3x+y-18=0$の制約下での極値を与える$x,y$は$x= \underline{\qquad} $, $y= \underline{\qquad}$である。
\item 解答例：ラグランジュ乗数を$\lambda$として、拡大関数は
$f_a = x^2+y^2 + \lambda(3x+y-18)$となり
\begin{eqnarray}
f_{ax} &=& 2x + 3\lambda = 0 \nonumber \\
f_{ay} &=& 2y + \lambda =0 \nonumber
\end{eqnarray}
なので$y=\frac{1}{3}x$が得られ拘束条件に代入すると$x=\frac{27}{5}=5.4$, $y=\frac{9}{5}=1.8$の時、極小値$\frac{27^2+9^2}{25} = \frac{810}{25} = \frac{162}{5} = 32.4$
\end{itemize}
\end{slide}

\begin{slide}{Quiz12-3}
\begin{itemize}
\item 領域$D$で重積分せよ。$\iint_D xy dxdy \qquad D: (x-1)^2+ y^2 \leqq 4$
\item 解答例：
\begin{eqnarray}
&\int_{-1}^3& \int_{-\sqrt{4-(x-1)^2}}^{\sqrt{4-(x-1)^2}} xy dy dx= \int_{-1}^3 x\frac{1}{2}\left[y^2\right]_{-\sqrt{4-(x-1)^2}}^{\sqrt{4-(x-1)^2}} dx \nonumber \\
&=& \int_{-1}^3 x0dx = 0 \nonumber
\end{eqnarray}
\end{itemize}
\end{slide}
