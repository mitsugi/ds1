...
さらに速度の時間微分（位置の二階微分)$\frac{dv}{dt} = \frac{d^2x}{dt^2}$は加速度である。加速度に質量$m$を掛けると与える力$F$と等しいとするニュートンの運動方程式（第２法則）はよく知られている。
\begin{equation}
m\frac{d^2x}{dt^2} = F
\label{eq:newton}
\end{equation} 
...
\vspace{5mm}
特別な問題を除けば、$F$が時間で変化する場合には、式の形で積分を計算できない（定積分）。コンピュータを使って積分の近似値を求める手法（シミュレーション:simulation)が様々開発され、利用されている。

\vspace{5mm}

たとえば人工衛星の地球を中心とした二次元運動$(x, y)$を考える場合、重力は地球の質量$M$と、地球からの距離$r = \sqrt{x^2 + y^2}$に依存し、万有引力の法則(The law of universal gravity)を用いて、以下のように地球中心からの距離が変わると重力が変化するモデルとなる。
\begin{equation}
m(\frac{d^2 x}{dt^2}, \frac{d^2 y}{dt^2}) = F(x, y) = \frac{GMm}{r^3}(x, y)
\label{eq:ge}
\end{equation}
ここで$G$は次の万有引力定数である。
\begin{equation}
...
したがって$x$軸を鉛直とすると、地球上での運動方程式は、地球の半径$r_o$が約$6378$km, 地球の質量$M$=$5.97 \times 10^{24}$kgを代入して
\begin{equation}
m(\frac{d^2 x}{dt^2}, \frac{d^2 y}{dt^2})  =(-\frac{6.67 \times 10^{-11} 5.97 \times 10^{24}}{6378 \times 10^{6}}m, 0)\approx (-9.8m, 0)
\end{equation}
と良く知った形に導出できる。


式\ref{eq:ge}を時間に関して二階積分できれば、人工衛星の位置$x,y$が時間の関数として表すことができるはずだが、式\ref{eq:ge}を数式として不定積分することは一般には困難である。


そこで、本来時間的に連続している変数（たとえば位置$x$)を短い時間刻み($\Delta t$)間隔での離散値$x_i$でFig.~\ref{fig:discrete}のように折れ線近似できると考えて
\begin{figure}[hbt]
\centering
...
\begin{eqnarray}
v_i &=& \frac{dx}{dt}_i \approx \frac{x_{i+1} - x_{i}}{\Delta t} \\
a_i &=& \frac{d^2 x}{dt^2}_i \approx \frac{x_{i+1} -2x_{i}+ x_{i-1}}{(\Delta t)^2} \label{eq:acc}
\end{eqnarray} 

したがって現在、過去の位置２つ$x_i, x_{i-1}$を使った漸化式を運動方程式に当てはめる未来の$x_{i+1}$を計算できる。$y_i$も同様である。

たとえば人工衛星が地上550kmの軌道にあるとして、$r_i = \sqrt{x_i^2+y_i^2}$とし、式\ref{eq:ge}を式\ref{eq:acc}を用いて離散値で表すと
\begin{eqnarray}
(\frac{x_{i+1} -2x_{i}+ x_{i-1}}{(\Delta t)^2}) &=& \frac{GM}{r_i^3} x_i \\
(\frac{y_{i+1} -2y_{i}+ y_{i-1}}{(\Delta t)^2}) &=& \frac{GM}{r_i^3} y_i \\
\end{eqnarray}
式を変形すれば、未知数を左辺に置いた以下を得る。
\begin{eqnarray}
x_{i+1} &=& (\Delta t)^2\frac{GM}{r_i^3} x_i + 2x_i - x_{i-1} \\
y_{i+1} &=&  (\Delta t)^2\frac{GM}{r_i^3} y_i + 2y_i - y_{i-1}\\
\end{eqnarray}

この式を初速を変えて$\Delta t = 10$秒で順次計算した結果が図\ref{fig:starlink}である。
\begin{figure}[hbt]
\centering
\includegraphics[width=\textwidth]{starlink.eps}
\caption{数値積分による人工衛星の軌道計算}
\label{fig:starlink}
\end{figure}
初速度$7.5 km/s$の場合、ほぼ円軌道になり周回速度は約6000秒=1時間40分、それより早いと楕円軌道、それより遅いと、地球に墜落してしまう。
この計算はstarlinkの衛星軌道にほぼ合致している。

この数値積分プログラムは次のMATLABのmスクリプト47行で作成した。
...
drawcircle('Center', [0, 0], 'Radius', ro/1000, 'Color', 'black'); 
a1 = animatedline('Color', 'blue', "lineWidth",  2, 'MaximumNumPoints',100);

% initial values
x = [550000+ro,  550000+ro]; 
y = [0, deltat*7500]; 
xxp = []; yyp=[]; 
hold off; 

an = annotation('textbox', [0.7, 0.8, 0.1, 0.1], "String",''); 
an.LineStyle="none"; 
for i=1:10000
    % stop if the satellite falls in the earth
    if (sqrt(x(2)*x(2)+y(2)*y(2)) < 1000000)
        break;
    end
   ctime = i*deltat; 
   % time stamp update
   if (mod(i, 100) == 0)
   an.String=string(ctime); 
   end
   % integration
   xp = -deltat^2*G*M/power(sqrt(x(2)*x(2)+y(2)*y(2)), 3)*x(2) + 2*x(2) - x(1); 
   yp = -deltat^2*G*M/power(sqrt(x(2)*x(2)+y(2)*y(2)), 3)*y(2) + 2*y(2) - y(1); 
   addpoints(a1, xp/1000, yp/1000); 
   xxp = [xxp xp/1000]; 
   yyp = [yyp yp/1000];
   x = [x(2) xp];
   y = [y(2) yp];
